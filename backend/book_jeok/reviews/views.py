...
class ReviewById(APIView):
    ...
    def put(self, request, review_id):
        ...
        serializer = ReviewSerializerSave(review, data=request.data)
...
class CommentByReviewId(APIView):
    ...
    def post(self, request, review_id):
        ...
        comment_data = {'user_id': request.data.get('user_id'), 'review_id': review_id
...
class CommentById(APIView):
    ...
    def put(self, request, comment_id):
        ...
        serializer = CommentSerializerSave(comment, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        else:
    ...
    def delete(self, request, comment_id):
        comment = comment = Comment.objects.get(comment_id = comment_id)
        comment.delete()
        return Response(status=status.HTTP_200_OK)
